...
class SklearnLearner extends Learner {  
  ...
  /**
   * Gives information about the minImpuritySplit.
   */
  function getMinImpuritySplit() : ?float {
    return $this->minImpuritySplit;
  }

  /**
   * Sets the minImpuritySplit used.
   */
  function setMinImpuritySplit(?float $minImpuritySplit) :void {
  ...
  function teach(DiscriminativeModel &$model, Instances $data) {
    ...
    if ($classifier === "CART") {
      $command = escapeshellcmd("python DiscriminativeModel/PythonLearners/sklearn_learner.py "
                                ...
                                . $getParameter($minImpuritySplit) . " " . $getParameter($classWeight) . " "
                                . $getParameter($ccpAlpha));
      $output = shell_exec($command);
    } else {
      die_error("The classifier $classifier is invalid. Only CART is supported at the moment." . PHP_EOL);
    }
